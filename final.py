...
class Simulator(tkinter.Frame):
	"""docstring for Simulator"""
	def __init__(self, *args, **kwargs):
		tkinter.Frame.__init__(self, *args, **kwargs)
		self.EpisodeG = EpisodeGen()
		self.EpisodeG.start()
		# VARIABLES
		self.Number_of_roads = 2
		self.Number_of_lanes = 3
		self.Width_of_lane   = 20
		self.canvas_width    = 800
		self.canvas_height   = 500
		# END VARIABLES
		self.i=0
		self.canvas = tkinter.Canvas(width=800, height=500)
	...
	def draw_car(self,position):
		self.canvas.create_rectangle(self.canvas_width/2+position[0], self.canvas_height/2+position[1], self.canvas_width/2+self.Width_of_lane/2+position[0], self.canvas_height/2+self.Width_of_lane/2+position[1], fill="red", outline = 'grey')
	def draw_roads(self,angle=0):
		for i in range(self.Number_of_roads):
			angle = 180*i/self.Number_of_roads
			angle = math.radians(angle)
			# SEPARATORS
			self.canvas.create_line(self.canvas_width/2 + self.canvas_width*math.cos(angle),self.canvas_height/2 + self.canvas_width*math.sin(angle),self.canvas_width/2 - self.canvas_width*math.cos(angle),self.canvas_height/2 - self.canvas_width*math.sin(angle), fill="#333", width=self.Number_of_lanes*self.Width_of_lane*2)
		...
		for i in range(self.Number_of_roads):
			...
			for i in range(self.Number_of_lanes):
				self.canvas.create_line(self.canvas_width/2 + self.canvas_width*math.cos(angle) + (self.Width_of_lane*i - self.Width_of_lane*self.Number_of_lanes + self.Width_of_lane/2 + i)*math.sin(angle),
					self.canvas_height/2 + self.canvas_width*math.sin(angle) - (self.Width_of_lane*i - self.Width_of_lane*self.Number_of_lanes + self.Width_of_lane/2 + i)*math.cos(angle),
					self.canvas_width/2 - self.canvas_width*math.cos(angle) + (self.Width_of_lane*i - self.Width_of_lane*self.Number_of_lanes + self.Width_of_lane/2 + i)*math.sin(angle),
					self.canvas_height/2 - self.canvas_width*math.sin(angle) - (self.Width_of_lane*i - self.Width_of_lane*self.Number_of_lanes + self.Width_of_lane/2 + i)*math.cos(angle), fill="black", width=self.Width_of_lane)
			# RIGHT ROAD
			for i in range(self.Number_of_lanes):
				self.canvas.create_line(self.canvas_width/2 + self.canvas_width*math.cos(angle) + (self.Width_of_lane*(i+1) - self.Width_of_lane/2 + i)*math.sin(angle),
					self.canvas_height/2 + self.canvas_width*math.sin(angle) - (self.Width_of_lane*(i+1) - self.Width_of_lane/2 + i)*math.cos(angle),
					self.canvas_width/2 - self.canvas_width*math.cos(angle) + (self.Width_of_lane*(i+1) - self.Width_of_lane/2 + i)*math.sin(angle),
					self.canvas_height/2 - self.canvas_width*math.sin(angle) - (self.Width_of_lane*(i+1) - self.Width_of_lane/2 + i)*math.cos(angle), fill="black", width=self.Width_of_lane)
		
		for i in range(self.Number_of_roads):
			angle = 180*i/self.Number_of_roads
			angle = math.radians(angle)	
			# Middle Separator
			self.canvas.create_line(self.canvas_width/2 + self.canvas_width*math.cos(angle),self.canvas_height/2 + self.canvas_width*math.sin(angle),self.canvas_width/2 - self.canvas_width*math.cos(angle),self.canvas_height/2 - self.canvas_width*math.sin(angle), fill="#a09999", width=2)
	def start(self):
		self.draw_background()
		self.draw_roads()
		print('Action',self.i)
		self.EpisodeG.saveinfile('result.txt',"%120s   %20s   %5s"%(self.EpisodeG.get_state(),self.EpisodeG.action(),self.EpisodeG.getReward()))
	def run(self):
		self.start()
		self.draw_car((10,self.i))
		self.i+=5
